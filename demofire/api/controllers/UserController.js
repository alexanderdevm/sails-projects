...
module.exports = {
    signup: function (req, res) {
        ...
        if (req.param('password').length < 6) {
            return res.badRequest('Password must be as least 6 characters!');
        }
        ...
        if (!_.isString(req.param('username')) || req.param('username').match(
                /[^a-z0-9]/i)) {
            return res.badRequest(
                'Invalid username: must consist of number and letters only.');
        }
        ...
        Emailaddresses.validate({
            ...
            invalid: function () {
                return res.badRequest(
                    'Doesn\'t look like and email address to me!'
                );
            },
            success: function () {
                        ...
                        User.create(options).exec(function (err, createdUser) {
                            if (err) {
                                ...
                                if (err.invalidAttributes && err.invalidAttributes
                                    .username && err.invalidAttributes
                                    .username[0] && err.invalidAttributes
                                    .username[0].rule === 'unique') {
                                    return res.alreadyInUse(err);
                                }
